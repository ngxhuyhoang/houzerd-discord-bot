...
async function execute(message, serverQueue) {
    ...
    if (!voiceChannel)
        return message.channel.send("Chưa vào voice channel thì sao nghe làm sao được mà mở nhạc");
...
function skip(message, serverQueue) {
    if (!message.member.voice.channel)
        return message.channel.send("Không có ở trong voice channel thì không stop được");
    if (!serverQueue)
...
function stop(message, serverQueue) {
    if (!message.member.voice.channel)
        return message.channel.send("Không có ở trong voice channel thì không stop được");
    if (!serverQueue)
        return message.channel.send("Làm gì có nhạc mà dừng");
    serverQueue.songs = [];
    serverQueue.connection.dispatcher.end();
}
